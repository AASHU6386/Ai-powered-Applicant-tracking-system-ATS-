    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-wrap">
          <section className="welcome-row"><div><p className="eyebrow">Tuesday, August 25, 2026</p><h1>Good morning, Alex <span className="wave">✦</span></h1><p className="subtitle">Here’s what’s happening with your hiring pipeline today.</p></div><button className="primary-button"><Plus size={17} /> Add candidate</button></section>
          ...
          <section className="candidate-section"><div className="section-heading"><div><h2>Recent candidates</h2><p>Review and manage your latest applicants.</p></div><button className="text-button">View all candidates <ChevronRight size={16} /></button></div><div className="toolbar"><div className="search-box"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search candidates..." /></div><div className="stage-tabs">{stages.map((stage) => <button key={stage} className={activeStage === stage ? 'stage-tab active' : 'stage-tab'} onClick={() => setActiveStage(stage)}>{stage.replace(' candidates', '')}</button>)}</div><button className="filter-button"><Filter size={16} /> Filters</button></div><div className="candidate-table"><div className="table-header"><span>Candidate</span><span>Stage</span><span>Match score</span><span>Applied</span><span /></div>{visibleCandidates.map((candidate) => <div className="candidate-row" key={candidate.name}><div className="candidate-cell"><span className={`candidate-avatar ${candidate.tone}`}>{candidate.initials}</span><span><strong>{candidate.name}</strong><small>{candidate.role} · {candidate.location}</small></span></div><span className={`status-pill ${candidate.stage.toLowerCase()}`}>{candidate.stage}</span><span className="score"><Sparkles size={14} /> {candidate.score}%</span><span className="applied-time">{candidate.time}</span><div className="row-actions"><button className={shortlisted.includes(candidate.name) ? 'star-button selected' : 'star-button'} aria-label={`Shortlist ${candidate.name}`} onClick={() => toggleShortlist(candidate.name)}><Star size={17} fill={shortlisted.includes(candidate.name) ? 'currentColor' : 'none'} /></button><button className="more-button" aria-label={`More actions for ${candidate.name}`}><MoreHorizontal size={17} /></button></div></div>)}{visibleCandidates.length === 0 && <div className="empty-state">No candidates match your search.</div>}</div></section>
